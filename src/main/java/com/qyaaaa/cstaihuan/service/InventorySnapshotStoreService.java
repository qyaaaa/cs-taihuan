...
public class InventorySnapshotStoreService {
    ...
    public List<BuffItem> loadItems(long snapshotId) {
        return jdbcTemplate.query(
            "SELECT asset_id, goods_id, name, price, float_value, image_url, wear_name, collection_name, rarity, quality_label, tradable, raw_json FROM inventory_item WHERE snapshot_id = ? ORDER BY id ASC",
    ...
    public InventorySnapshotRecord saveSnapshot(String game, String fingerprint, List<BuffItem> items) {
        ...
        if (!items.isEmpty()) {
            jdbcTemplate.batchUpdate(
                "INSERT INTO inventory_item (snapshot_id, asset_id, goods_id, name, price, float_value, image_url, wear_name, collection_name, rarity, quality_label, tradable, raw_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                new BatchPreparedStatementSetter() {
                    public void setValues(PreparedStatement statement, int index) throws java.sql.SQLException {
                        ...
                        statement.setString(8, item.getWearName());
                        statement.setString(9, item.getCollection());
                        statement.setString(10, item.getRarity());
                        statement.setString(11, item.getQualityLabel());
                        statement.setBoolean(12, item.isTradable());
                        statement.setString(13, writeRaw(item.getRaw()));
                    }
