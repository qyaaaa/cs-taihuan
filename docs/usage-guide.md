...
拉取完成后会自动做两件事（不阻塞任务本身）：

- 按 `asset_id` 从上一快照**结转**已精估的材料市值（零请求）。
- 提交后台线程 `[float-refine]` 对还没精估的材料按磨损精估（BUFF 限流时冷却后自动续跑，进度见后端日志）。
...
- `按磨损精估材料价`：对该方案 10 件材料即时查询挂单底价并写回（每件约数秒），完成后自动重算。
- `补全该方案收藏品产物`：产物数量比 BUFF 少时，按内置全量名单补抓缺失产物的全部磨损档，完成后自动重算。

材料行会同时显示计价（磨损精估价）与灰字「底价」（磨损档地板价）对照。
...
- 数据来源：开源数据集 [ByMykel/CSGO-API](https://github.com/ByMykel/CSGO-API) 的中英文 `skins.json`，按皮肤 `id` 合并出双语名 + 共享的 `min/max float` + 图标 + 所属收藏品上线时间，精简快照内置在 `src/main/resources/data/skin-float-range.json`，**完全离线**，不依赖运行时联网。
- 存储与导入：导入到独立表 `skin_float_range`；表为空时启动自动 seed，表内已有数据时启动会从快照补充缺失的 `skin_id`，并把图标/上线时间**同步为快照值**（快照是唯一真相源：修正快照后重启即自动传播；快照值为空时保留 DB 现值，不清空）。也可手动 `POST /api/skin-float-range/import` 补缺，`GET /api/skin-float-range/stats` 查看条数。
...
- 更新数据：从数据源重新拉取中英文 `skins.json`，合并生成精简快照替换内置文件（**先备份到 `data/backup/`**）后重启，启动逻辑自动补充新增皮肤并同步图标/日期修正；无需清库。

## 11. 收藏品图鉴

侧边栏「收藏品」页按收藏品浏览基准库全部产物。

- 左侧收藏品列表：收藏品名、**上线时间**、产物件数；搜索框固定在面板顶部，滚动列表时始终可见。
- 右侧详情：产物图标、品质（按档位配色）、每个皮肤自身的 Min/Max Float 区间条、图案编号；支持按名称/武器搜索和档位筛选。
- 上线时间来源：新收藏品来自 ByMykel `crates` 的 `first_sale_date`；上游无日期的老地图收藏品用人工核实（Valve 公告 / CS Wiki / SteamDB）的静态映射，维护在快照 JSON 中。「限定物品」是杂项发放集合，无统一上线时间，刻意留空。
- 该基准库同时服务汰换计算：归一化磨损公式的皮肤 paint 范围、产物补全用的收藏品全量名单都来自它。
...
计价用的是**按磨损精估的市值**（该件 float 所在 BUFF 磨损子区间的挂单底价），旁边灰字「底价」是该磨损档的目录地板价，用于对照。同一磨损档内低磨损有明显溢价，只按档底价会低估投入、虚高 EV。精估在拉取库存后由后台线程自动进行（限流会冷却续跑），也可在方案详情点「按磨损精估材料价」即时补。

### 方案产物数量为什么比 BUFF 显示的少？

目录里该收藏品的产物档不全。点方案详情的「补全该方案收藏品产物」，系统会按内置全量名单找出缺口、去 BUFF 按名搜索并补抓全部磨损档，完成后自动重算。
