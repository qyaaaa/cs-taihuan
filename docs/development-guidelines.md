...
- `src/main/resources/data/skin-float-range.json` 是皮肤磨损范围/图标/收藏品上线时间的唯一数据源；Flyway 只管表结构，不写数据迁移。
- 修改快照前先备份到 `data/backup/`（带时间戳后缀）。
- 启动种子逻辑：新增行自动追加；图标/上线日期按 skin_id **同步为快照值**（修正快照后重启即传播；快照空值不清空 DB 现值）。
- 新增收藏品：从 ByMykel 上游按 `id` 差集生成条目追加进快照即可，重启自动增量导入。
- 人工维护的日期（老收藏品上线时间等）必须有外部来源核实（Valve 公告 / CS Wiki / SteamDB），不凭记忆填写。
